...
# print(tag_word)
# print(tag_counts)
fhandle = open('hmmmodel_test.txt', 'w')
...
for cur_tag in transition_prob:
    for prev_tag in tag_frequency_map:
        ...
        # adding add one smoothing
            ...
            transition_prob[cur_tag][prev_tag] = (transition_prob[cur_tag][prev_tag] + 1) / (
                        tag_frequency_map[prev_tag] + (4 * len(tag_frequency_map)) - 1)

model = {'tags': tag_frequency_map, 'transition': transition_prob, 'emission': emission_prob,
         'most_tag_word': send_tags}
